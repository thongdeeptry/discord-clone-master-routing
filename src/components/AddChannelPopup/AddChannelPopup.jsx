    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" classes={{ paper: classes.paper}}>
        <DialogContent className={classes.dialog}>
          <DialogContentText>
            <Typography variant='h5' className={classes.typo}>Tên kênh</Typography>
          </DialogContentText>
          <input type="text" required className='input' autoFocus placeholder='Nhập tên kênh' onChange={(e)=>setformValue(e.target.value)} value={formValue}/>
        </DialogContent>
        <DialogActions className={classes.dialogaction}>
            <form onSubmit={handleSubmit} className='form'>
              <button type='submit' onClick={handleClose} className='form__button'>Thêm vào</button>
            </form>
        </DialogActions>
      </Dialog>
    </div>
